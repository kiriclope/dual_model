...
with pgb.tqdm_joblib( pgb.tqdm(desc='computing m1 and phi', total=ini_list.shape[0] ) ) as progress_bar: 
    ...
    time_list, m0_list, m1_list, phi_list = zip( *Parallel(n_jobs=-2)(delayed(parloop)(gv.path, ini_id, MAP=gv.MAP) 
...
avg_phi = circmean(phi_list, high=np.pi, axis=-1) 
...
ax1.set(ylim=(0, np.ceil( np.nanmax(m0_list) * 12 )/10 ) ) 
ax2.set(ylim=(0, np.ceil( np.nanmax(m1_list) * 12 )/10 ) )  
