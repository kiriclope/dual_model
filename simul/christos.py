...
if __name__ == "__main__":
        ...
        for n_session in range(1, 50):
            ...
            try:
                ...
                for i_stim in range(thetas_in.shape[0]):
                    drift_loc.append( np.nanmean(get_drift(thetas_out[i_stim], thetas_in[i_stim], THRESH) ) ) 
                    diff_loc.append( np.nanmean(get_diff(thetas_out[i_stim], thetas_in[i_stim], THRESH) ) ) 
    ...
    _, bins, _ = plt.hist(diff_off, histtype='step', color=pal[0],
                          lw=2, density=1, alpha=.5, bins='auto')
    
    
    _, bins, _ = plt.hist(diff_on, histtype='step', color=pal[1],
                          lw=2, density=1, alpha=.5, bins='auto')

    bins_fit = np.linspace(-THRESH, THRESH, 100)
    mu_off, sigma_off = stat.norm.fit(diff_off)
    fit_off = stat.norm.pdf(bins_fit, mu_off, sigma_off)
    plt.plot(bins_fit, fit_off, color=pal[0], lw=2)

    mu_on, sigma_on = stat.norm.fit(diff_on)
    fit_on = stat.norm.pdf(bins_fit, mu_on, sigma_on)
    plt.plot(bins_fit, fit_on, color=pal[1], lw=2)
    ...
    stats = stat.ttest_ind(np.abs(diff_off), np.abs(diff_on), nan_policy='omit', equal_var=False)
    print('pval', stats.pvalue)
