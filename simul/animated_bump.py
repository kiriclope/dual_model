...
fig, (ax1, ax2, ax3) = plt.subplots(
    1, 3, figsize=(1.25 * 1.618 * 2 * 3, 1.618 * 2), num=figname
)
...
for i_pop in range(gv.n_pop):
    ...
    if i_pop == 0:
        smooth_line_E = ax1.plot(
            theta, smooth_rates[i_pop, 0, : gv.n_size[i_pop]], "-", color=gv.pal[i_pop]
        )[0]
    else:
        smooth_line_I = ax1.plot(
            theta, smooth_rates[i_pop, 0, : gv.n_size[i_pop]], "-", color=gv.pal[i_pop]
        )[0]
...
for i_pop in range(gv.n_pop):
    if i_pop == 0:
        phi_line_E = ax3.plot(
            time[0], phi[i_pop, 0], "-", color=gv.pal[i_pop], alpha=alpha[i_pop]
        )[0]
...
# # run the animation
anim = FuncAnimation(
    fig, animate, frames=time.shape[0], interval=50, repeat=True, cache_frame_data=False
)
plt.draw()
plt.show()
