...
with pgb.tqdm_joblib( pgb.tqdm(desc='phi off', total= gv.N_INI*gv.N_TRIALS) ) as progress_bar: 
    ...
    m1_off, phi_off = zip( *Parallel(n_jobs=-64)(delayed(parloop)(path, i_trial, i_ini, A_CUE=.25, EPS_CUE=.25)
...
with pgb.tqdm_joblib( pgb.tqdm(desc='phi on', total= gv.N_INI *gv.N_TRIALS) ) as progress_bar: 
    ...
    m1_on, phi_on = zip( *Parallel(n_jobs=-64)(delayed(parloop)(path, i_trial, i_ini, A_CUE=.25, EPS_CUE=.25) 
                                               for i_ini in range(1, gv.N_INI+1) 
                                               for i_trial in range(1, gv.N_TRIALS+1) ) 
    ) 
...
sem = [scipy.stats.sem(2*drift_off_stack, ddof=1, nan_policy='omit'),
...
mean_phi_off = stat.circmean(phi_off, high=180, low=0, nan_policy='omit', axis=0) # average over initial conditions
...
mean_phi_on = stat.circmean(phi_on, high=180, low=0, nan_policy='omit', axis=0) # average over initial conditions
...
_, bins_off, _ = plt.hist(2*diff_off_stack, histtype='step', color=pal[0],
                          density=1, alpha=.5, bins='auto')
_, bins_on, _ = plt.hist(2*diff_on_stack, histtype='step', color=pal[1],
                         density=1, alpha=.5, bins='auto')
...
sem = [scipy.stats.sem(abs_diff_off, ddof=1, nan_policy='omit'),
