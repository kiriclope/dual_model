...
def get_m1_phi_smooth_rates(rates, n_size=gv.n_size, window=0.1):
    ...
    for i_pop in range(rates.shape[1]):
        ...
        smooth_rates[i_pop, :, : n_size[i_pop]] = circular_convolution(
            pop_rates, int(pop_rates.shape[-1] * window)
        )
