...
trial_list = np.arange(0, 5, 1) 
gv.IF_INI_COND = 0 
gv.IF_TRIALS = 0 
gv.init_param()

def parloop(path, trial_id, con_path=gv.con_path, MAP=0, verbose=0): 
...
with pgb.tqdm_joblib( pgb.tqdm(desc='computing m1 and phi', total=trial_list.shape[0] ) ) as progress_bar: 
    
    time_list, m0_list, m1_list, phi_list = zip( *Parallel(n_jobs=-2)(delayed(parloop)(gv.path, trial_id, MAP=gv.MAP) 
...
figtitle = 'phi_vs_trial_' + gv.folder + '_MAP_%d' % gv.MAP 
...
for i_pop in range(gv.n_pop): 
    ax1.plot(time_list.T, m0_list[i_pop].T, color= gv.pal[i_pop], alpha=0.1) 
ax1.set_xlabel('Time (s)') 
ax1.set_ylabel('$\\nu^{(0)}_%d$ (Hz)' % gv.MAP) 

for i_pop in range(gv.n_pop): 
    ax2.plot(time_list.T, m1_list[i_pop].T, color= gv.pal[i_pop], alpha=0.1) 
ax2.set_xlabel('Time (s)') 
ax2.set_ylabel('$\\nu^{(1)}_%d$ (Hz)' % gv.MAP) 

for i_pop in range(gv.n_pop):
    ax3.plot(time_list.T, phi_list[i_pop].T, color= gv.pal[i_pop], alpha=0.1) 
...
for i_pop in range(gv.n_pop): 
    ax4.hist(avg_m0[i_pop], color= gv.pal[i_pop], alpha=0.5) 
ax4.set_xlabel('$\\nu^{(0)}_%d$ (Hz)' % gv.MAP) 
ax4.set_ylabel('Count') 

for i_pop in range(gv.n_pop): 
    ax5.hist(avg_m1[i_pop], color= gv.pal[i_pop], alpha=0.5) 
ax5.set_xlabel('$\\nu^{(1)}_%d $ (Hz)' % gv.MAP) 
ax5.set_ylabel('Count') 

for i_pop in range(gv.n_pop): 
    ax6.hist(avg_phi[i_pop], color= gv.pal[i_pop], alpha=0.5) 
